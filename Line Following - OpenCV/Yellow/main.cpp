...
int main( int argc, char** argv )
{
    ...
    while(1)    // Main loop to perform image processing
    {
        ...
        // Threshold the image to extract the yellow pixels
        cv::Mat mask1, mask2;
        cv::inRange(hsv, cv::Scalar(20, 70, 70), cv::Scalar(30, 255, 255), mask1);
        cv::inRange(hsv, cv::Scalar(30, 70, 70), cv::Scalar(40, 255, 255), mask2);
        ...
        // Draw the contours on the original image ( Yellow)
        ...
        cv::Mat bottomRow = mask.row(mask.rows-50);
            ...
            // Find the range of yellow pixels
            int start = -1, end = -1;
            for (int i = 0; i < colors.size(); i++) {
                cv::Vec3b color = colors[i];
            if (color[2] > 150 && color[1] > 150 && color[0] < 100) {
                if (start == -1) {
                start = i;
                }
                end = i;

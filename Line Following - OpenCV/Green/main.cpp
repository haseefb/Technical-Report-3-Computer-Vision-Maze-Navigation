...
int main( int argc, char** argv )
{
    ...
    while(1)    // Main loop to perform image processing
    {
        ...
        // Threshold the image to extract the green pixels
        cv::Mat mask1, mask2, mask3;
        cv::inRange(hsv, cv::Scalar(25, 70, 70), cv::Scalar(65, 255, 255), mask1);
        cv::inRange(hsv, cv::Scalar(65, 70, 70), cv::Scalar(85, 255, 255), mask2);
        cv::inRange(hsv, cv::Scalar(85, 70, 70), cv::Scalar(95, 255, 255), mask3);
        ...
        Draw the contours on the original image (Green)
        ...
        cv::Mat bottomRow = mask.row(mask.rows-50);
          ...
          // Find the range of green pixels
            int start = -1, end = -1;
            for (int i = 0; i < colors.size(); i++) {
            cv::Vec3b color = colors[i];
            if (color[2] < 100 && color[1] > 150 && color[0] < 100) {
            if (start == -1) {
            start = i;
                }
            end = i;

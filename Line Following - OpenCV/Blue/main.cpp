...
int main( int argc, char** argv )
{
    ...
    while(1)    // Main loop to perform image processing
    {
        ...
        cv::Mat mask1, mask2, mask3, mask4, mask5, mask6, mask7, mask8, mask9, mask10;
        cv::inRange(hsv, cv::Scalar(100, 70, 70), cv::Scalar(130, 255, 255), mask1);
        cv::inRange(hsv, cv::Scalar(130, 70, 70), cv::Scalar(150, 255, 255), mask2);
        cv::inRange(hsv, cv::Scalar(80, 70, 70), cv::Scalar(100, 255, 255), mask3);
        cv::inRange(hsv, cv::Scalar(150, 70, 70), cv::Scalar(170, 255, 255), mask4);
        cv::inRange(hsv, cv::Scalar(0, 70, 70), cv::Scalar(20, 255, 255), mask5);
        cv::inRange(hsv, cv::Scalar(212, 65, 27), cv::Scalar(217, 76, 16), mask6);
        cv::inRange(hsv, cv::Scalar(100, 0, 0), cv::Scalar(130, 70, 70), mask7);
        cv::inRange(hsv, cv::Scalar(130, 0, 0), cv::Scalar(150, 70, 70), mask8);
        cv::inRange(hsv, cv::Scalar(170, 70, 70), cv::Scalar(180, 255, 255), mask9);
        cv::inRange(hsv, cv::Scalar(20, 70, 70), cv::Scalar(40, 255, 255), mask10);

        cv::Mat mask = mask1 | mask2 | mask3 | mask4 | mask5 | mask6 | mask7 | mask8 | mask9 | mask10;
        ...
        Draw the contours on the original image (Blue)
        ...
        cv::Mat bottomRow = mask.row(mask.rows-50);
            ...
            // Find the range of blue pixels
            int start = -1, end = -1;
            for (int i = 0; i < colors.size(); i++) {
                cv::Vec3b color = colors[i];
                if (color[0] > 150 && color[1] < 100 && color[2] < 100) {
                    if (start == -1) {
                        start = i;
                    }
                    end = i;

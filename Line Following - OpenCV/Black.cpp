...
int main( int argc, char** argv )
{
    ...
    while(1)    // Main loop to perform image processing
    {
        ...
        // Threshold the image to extract the black pixels
        cv::Mat mask1, mask2, mask3;
        cv::inRange(hsv, cv::Scalar(0, 0, 0), cv::Scalar(180, 255, 30), mask1);
        cv::inRange(hsv, cv::Scalar(0, 0, 0), cv::Scalar(180, 50, 255), mask2);
        cv::inRange(hsv, cv::Scalar(0, 0, 0), cv::Scalar(206, 28, 10), mask2);
        ...
        Draw the contours on the original image (White)
        cv::drawContours(frame, contours, -1, cv::Scalar(255, 255, 255), 2);
        ...
        // Find the white pixel in the bottom row (Black)
            int start = -1;
            int end = -1;
            for (int i = 0; i < bottomRow.cols; i++) {
                if (bottomRow.at<uchar>(0, i) == 255) {
                    if (start == -1) {
                        start = i;
                    }
                    end = i;
